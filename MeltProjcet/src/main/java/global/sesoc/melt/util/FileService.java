package global.sesoc.melt.util;
...
public class FileService {
	...
	public static String saveFile(MultipartFile upload, String uploadPath) {
		...
		if(oprofile.trim().length() == 0 || upload.isEmpty()) 
			return "";
		...
		String ext;				// 확장명
		String sprofile;	// 디스크에 저장할 이름
		
		int lastIndex = oprofile.lastIndexOf('.');
		...
		//확장자가 있는 경우
			...
			ext = "." + oprofile.substring(lastIndex + 1);
			filename= oprofile.substring(0, lastIndex);
		}
		 ...
		 * while (true) {
		 * 
		 * //같은 이름의 파일이 없으면 나감. if ( !serverFile.isFile()) break; //같은 이름의 파일이 있으면 이름 뒤에
		 * long 타입의 시간정보를 덧붙임. savedFilename = filename + new Date().getTime(); }
		 */
